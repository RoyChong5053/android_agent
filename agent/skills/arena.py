...
from agent.capture import capture_frame
from agent.detectors import find_template
from agent.journal import Journal
from agent.screens import ANCHORS, detect_state, get_template
from agent.vl_sweep import parse_list
...
def _progressing(secs: float = 15) -> bool:
    ...
    """
    from agent.state import frame_diff
    f0 = capture_frame()
    hits = 0
    n = max(2, int(secs / 3))
    for _ in range(n):
